...
export default function BlogSlider({posts, title = "Noticias y Anuncios"}: Props) {
    ...
    return (
        <section className="w-full flex flex-col items-center">
            <h2 className="text-5xl text-center font-avenir font-bold mb-6">Visita nuestro blog</h2>
      ...
      `}
                    ...
                    {posts.map((post) => (
                        <Link
                            ...
                            state={post}
                            className="snap-start shrink-0 w-[95%] sm:w-[70%] md:w-[45%] lg:w-[32%]
                                       bg-white rounded-3xl border border-stone-200 hover:border-stone-500
                                       hover:shadow-md overflow-hidden transition-all duration-200 min-h-[380px] md:min-h-[420px] "
                        >
                            <img
                                src={post.imageUrl}
                                alt={post.title}
                                className="w-full h-48 md:h-58 object-cover"
                            />
                            <div className="p-4">
                                <h3 className="text-xl md:text-2xl font-title-bold mb-1 line-clamp-4">
                                    {post.title}
                                </h3>
